...
D.extend({
  ...
  qsa: function qsa(element, selector) {
    ...
    return (
      ...
      isSimple && !maybeID && element.getElementsByClassName ? maybeClass
...
function offsetParent() {
  return this.map(function () {
    ...
    while (parent && !rootNodeRE.test(parent.nodeName) && D(parent).css('position') == 'static') parent = parent.offsetParent;
    return parent;
  });
}
...
function attr(name, value) {
  ...
  return typeof name == 'string' && !(1 in arguments) ? 0 in this && this[0].nodeType == 1 && (result = this[0].getAttribute(name)) != null ? result : undefined : this.each(function (idx) {
    ...
    if (isObject(name)) for (var key in name) setAttribute(this, key, name[key]);else setAttribute(this, name, funcArg(this, value, idx, this.getAttribute(name)));
  });
}
...
function prop(name, value) {
  ...
  return typeof name == 'string' && !(1 in arguments) ? this[0] && this[0][name] : this.each(function (idx) {
    if (isObject(name)) for (var key in name) this[propMap[key] || key] = name[key];else this[name] = funcArg(this, value, idx, this[name]);
  });
}
...
function closest(selector, context) {
  ...
  this.each(function (_, node) {
    while (node && !(collection ? collection.indexOf(node) >= 0 : D.matches(node, selector))) node = node !== context && !isDocument(node) && node.parentNode;
...
function parents(selector) {
  var ancestors = [],
    nodes = this;
  while (nodes.length > 0) nodes = D.map(nodes, function (node) {
...
function anim$1(properties, duration, ease, callback, delay) {
  ...
  if (duration > 0) {
    this.on(endEvent, _wrappedCallback);
